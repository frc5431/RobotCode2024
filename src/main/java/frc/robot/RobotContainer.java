...
public class RobotContainer {
  ...
  public RobotContainer() {
    intakeNote = DependantsResolver.collapse(systems.getIntake(), systems.getPivot(),
        (intake, pivot) -> new IntakeNote(intake, pivot), () -> null);
    NamedCommands.registerCommand("AmpScore", DependantsResolver.collapse(
        systems.getShooter(), systems.getIntake(),
        (shooter, intake) -> new AmpScore(shooter, intake),
        () -> new InstantCommand()));
    NamedCommands.registerCommand("ShooterRev",
        DependantsResolver.collapse(systems.getShooter(),
            (shooter) -> new StartEndCommand(
                () -> shooter.runPair(ShooterModes.SpeakerDistant.speed, shooter.distantTopController,
                    shooter.distantBottomController),
                () -> {
                }).withTimeout(3)));
    NamedCommands.registerCommand("SpeakerScore", DependantsResolver.collapse(systems.getShooter(), systems.getIntake(),
        (shooter, intake) -> new SimpleSpeaker(shooter, intake)));
    NamedCommands.registerCommand("DistantSpeakerScore", DependantsResolver.collapse(systems.getShooter(),
        systems.getIntake(), (shooter, intake) -> new DistantSpeakerScore(shooter, intake)));
    NamedCommands.registerCommand("GrabNote", DependantsResolver.collapse(systems.getIntake(), systems.getPivot(),
        (intake, pivot) -> new AutoIntakeNote(intake, pivot)));
  ...
  public void periodic() {
    ...
    if (systems.getShooter().isPresent() && systems.getPivot().isPresent()) {
      var shooter = systems.getShooter().get();
      var pivot = systems.getPivot().get();
      ...
      if (shooter.mode == ShooterModes.NONE) {
        ...
        if (systems.getAmperPivot().isPresent()) {
          systems.getAmperPivot().get().setpoint = AmperConstants.anglerConstants.minAngle;
        }
        intakeNote.cancel();
    ...
    if (systems.getIntake().isPresent()) {
      var intake = systems.getIntake().get();
  ...
  private Command lockToAngleCommand(double redAngle, double blueAngle) {
    return drivebase.applyRequest(() -> {
      ...
      return facingRequest
          ...
          .withHeading(edu.wpi.first.math.util.Units.degreesToRadians(
              (DriverStation.getAlliance().get() == DriverStation.Alliance.Blue) ? blueAngle : redAngle))
  ...
  private void configureBindings() {
    ...
    if (Constants.isGals) {
      ...
      // Amper
      if (systems.getAmperPivot().isPresent()) {
        var amperPivot = systems.getAmperPivot().get();
        o_incrementAmper
            .whileTrue(new RunCommand(() -> amperPivot.increment(operator.getRightY() * 1), amperPivot).repeatedly());
        o_decrementAmper
            .whileTrue(new RunCommand(() -> amperPivot.increment(operator.getRightY() * 1), amperPivot).repeatedly());
      }
    ...
    if (systems.getLeftClimber().isPresent() && systems.getRightClimber().isPresent()) {
      var rightClimber = systems.getRightClimber().get();
      var leftClimber = systems.getLeftClimber().get();

      d_incrementClimber
          .whileTrue(rightClimber.increment(-2).repeatedly().alongWith(leftClimber.increment(-2).repeatedly()));
    ...
    if (systems.getAmperPivot().isPresent() && systems.getPivot().isPresent()) {
      var amperPivot = systems.getAmperPivot().get();
      var pivot = systems.getPivot().get();
    ...
    // Shooter
    if (systems.getShooter().isPresent()) {
      var shooter = systems.getShooter().get();
      ...
      if (systems.getPivot().isPresent()) {
        o_deployIntake.onTrue(new RunAnglerCommand(RunAnglerCommand.AnglerModes.DEPLOY, systems.getPivot().get()));
      }
    ...
    if (systems.getIntake().isPresent()) {
      ...
      if (systems.getAmper().isPresent()) {
        var amper = systems.getAmper().get();
    ...
    if (systems.getPivot().isPresent()) {
      ...
      if (systems.getAmperPivot().isPresent()) {
        ...
        o_downPivots.onTrue(new RunAnglerCommand(RunAnglerCommand.AnglerModes.DEPLOY, pivot)
            .alongWith(
                new RunAnglerCommand(RunAnglerCommand.AnglerModes.STOW, amperPivot,
                    TerminationCondition.SETPOINT_REACHED))
            .andThen(new InstantCommand(() -> amperPivot.setpoint = Units.Degree.of(232))));
